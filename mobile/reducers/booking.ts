...
export function loadBookings() {
    ...
    const bookings = (await api.get('Booking')).data;
    for (let booking of bookings) {
      booking.route = (await api.get(`Route/${booking.route.split('#')[1]}`)).data;
      const routeElements = [];
      for (let elem of booking.route.routeElements) {
        const routeElement = (await api.get(`RouteElement/${elem.split('#')[1]}`)).data;
        if (booking.status === 'AUCTION') {
          const ft = {
            where: {
              route: `resource:org.travelchain.network.Route#${booking.route.routeId}`,
              routeElement: elem,
              booking: `resource:org.travelchain.network.Booking#${booking.bookingId}`
            }
          }
          routeElement.bets = (await api.get(`Bet?filter=${JSON.stringify(ft)}`)).data.length
        }
        routeElements.push(routeElement);
      }
      booking.route.routeElements = routeElements;
    }
...
export function finishBookingAuction(booking: string) {
	return async (dispatch: any) => {
    await api.post('FinishBookingAuction', { booking });
		await dispatch(loadBookings());
  }
}

export function finishContracts(booking: any) {
	return async (dispatch: any) => {
    console.log(booking);
    for (let contract of booking.contracts) {
      await api.post('FinishContract', { contract });
    }
		await dispatch(loadBookings());
  }
}
